...
### 서버 연결 스텁
- [ ] WebSocket 또는 HTTP 폴링 스텁 구현
- [ ] 테스트용으로 3초마다 0/1 토글 신호 생성
- [ ] `ALERT_FROM_SERVER` IPC 채널로 렌더러에 전송

### 블라인드 표시
- [ ] `harmful=true`일 때 ROI 사각형을 `rgba(0,0,0,0.4)` 반투명 블라인드로 채움
- [ ] 테두리는 유지 (선택 사항: 테두리 색상 변경)
- [ ] 모드를 `alert`로 전환

### 블라인드 해제
- [ ] `harmful=false`일 때 블라인드 제거
- [ ] 모드를 `detect`로 복귀 (테두리만 유지)
- [ ] CSS 트랜지션으로 150-250ms 페이드 효과
