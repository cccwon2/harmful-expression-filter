...
⚠️ 레거시 (현재는 서버 측 PaddleOCR 사용)

## 📋 작업 개요

**⚠️ 레거시**: 이 작업은 현재 사용하지 않는 Windows SDK OCR에 대한 최적화 작업입니다. 현재는 서버 측 PaddleOCR을 사용합니다.
...
- 서버 측 PaddleOCR 사용 시: Electron → 화면 캡처 → 서버 전송 → PaddleOCR 처리
- 자세한 내용은 [Task 28: PaddleOCR 서버 연동](./28-paddle-ocr-integration.md) 참조
