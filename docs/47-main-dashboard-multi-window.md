...
- ✅ `switchToOcrMode()` 함수 분리: OCR 모드 전환 로직을 별도 함수로 분리하여 재사용 가능하도록 개선
- ✅ 음성 모드에서 OCR 모드로 전환 시 자동으로 음성 스트리밍 중지
- ✅ 트레이 메뉴에서 "영역 지정" 클릭 시 OCR 모드로 직접 전환
- ✅ 트레이 메뉴에서 "영역 재지정" 클릭 시 OCR 모드로 직접 전환
- ✅ 메인 윈도우 자동 숨김: OCR 모드 전환 시 메인 윈도우를 자동으로 찾아서 숨김
...
- ✅ OCR 캡처 간격: 500ms → 800ms (성능 최적화)
- ✅ 썸네일 크기 최적화: 전체 화면 대신 ROI 영역 중심으로만 캡처
- ✅ 비동기 처리로 지연 최소화
- ✅ 유해 표현 감지 시에만 블러 처리
- ✅ 로그 최적화: 유해 표현 감지 위주로 출력
- ✅ 중복 텍스트 필터링 (OCR 및 서버 분석 중복 방지)
...
- ✅ `switchToOcrMode()` 함수 분리: OCR 모드 전환 로직을 별도 함수로 분리하여 트레이에서도 직접 호출 가능
- ✅ OCR 모드 선택 시 오버레이 윈도우 동적 생성
- ✅ 음성 모드에서 OCR 모드로 전환 시 자동으로 음성 스트리밍 중지
- ✅ 메인 윈도우 자동 숨김: mainWindow가 null이면 모든 윈도우에서 찾아서 숨김
...
- ✅ OCR 캡처 간격: 500ms → 800ms (성능 최적화)
- ✅ 썸네일 크기 최적화: ROI 영역 중심으로만 캡처하여 메모리 사용량 감소
- ✅ 크롭 좌표 스케일링: 썸네일 크기에 맞춰 ROI 좌표 조정
- ✅ 비동기 처리로 지연 최소화 (다음 캡처를 먼저 스케줄링)
- ✅ 중복 텍스트 필터링: `lastExtractedText`로 중복 OCR 결과 방지
- ✅ 단일 인스턴스 보장: `app.requestSingleInstanceLock()` 추가
...
- ✅ 트레이 메뉴 "영역 지정" 기능
- ✅ 트레이 메뉴 "영역 지정" 클릭 시 OCR 모드로 직접 전환 (`switchToOcrMode()` 호출)
- ✅ 트레이 메뉴 "영역 재지정" 클릭 시 OCR 모드로 직접 전환 (`switchToOcrMode()` 호출)
- ✅ 오버레이 표시/숨김 기능
- ✅ 스트리밍 상태 표시 (AudioManager 상태 연동)
- ✅ `setTrayAudioUpdateCallback` 함수로 트레이 업데이트 콜백 지원
...
- [x] 음성 모드 진입 시 트레이 메뉴에 스트리밍 상태가 표시되는가? ✅
- [x] 음성 모드에서 OCR 모드로 전환 시 음성 스트리밍이 자동으로 중지되는가? ✅
- [x] 트레이 메뉴 "영역 지정" 클릭 시 OCR 모드로 전환되는가? ✅
- [x] 트레이 메뉴 "영역 재지정" 클릭 시 OCR 모드로 전환되는가? ✅
- [x] OCR 모드 전환 시 메인 윈도우가 자동으로 숨겨지는가? ✅
- [x] 서버로부터 `is_harmful=true` 응답 시 블러 처리가 되는가? ✅
- [x] 중복 실행이 방지되는가? ✅

### 성능 검증 ✅

- [x] OCR 캡처 간격이 800ms로 설정되어 있는가? ✅
- [x] 썸네일 크기가 최적화되어 있는가? ✅
- [x] 서버 AI 분석 타임아웃이 3초로 설정되어 있는가? ✅
- [x] 유해 표현 감지 시에만 로그가 출력되는가? ✅
- [x] 중복 텍스트가 필터링되는가? ✅
- [x] 단일 인스턴스가 보장되는가? ✅
- [x] 음성 모드 진입 시 트레이 메뉴가 업데이트되는가? ✅
...
7. ✅ `electron/ipc/dashboardHandlers.ts`:
   ...
   - 모드 선택 IPC 핸들러 구현 (신규)
   - `switchToOcrMode()` 함수 분리: OCR 모드 전환 로직을 별도 함수로 분리
   - 음성 모드에서 OCR 모드로 전환 시 자동으로 음성 스트리밍 중지
   - 메인 윈도우 자동 숨김 로직 개선 (null인 경우 자동으로 찾아서 숨김)
   - 오버레이 윈도우 동적 생성
   - Ctrl+E/Q 단축키 등록
   - 트레이 아이콘 생성
...
11. ✅ `electron/tray.ts`:
    ...
    - 트레이 메뉴 "영역 지정" 클릭 시 `switchToOcrMode()` 직접 호출
    - 트레이 메뉴 "영역 재지정" 클릭 시 `switchToOcrMode()` 직접 호출
    - OCR 모드 전환 실패 시 폴백 로직 (기존 setup 모드 진입)

12. ✅ `electron/ipc/roi.ts`:

    - ROI 선택 시 `main.ts`의 `currentROI` 업데이트

13. ✅ `electron/main/AudioManager.ts`:

    - `startStream()` 완료 후 트레이 메뉴 자동 업데이트
    - `stopStream()` 완료 후 트레이 메뉴 자동 업데이트
    - 트레이 업데이트 콜백을 통한 상태 동기화

14. ✅ `electron/windows/createMainWindow.ts`:
    - 모드 선택 창 크기: 400x320 (트렌디한 UI)
    - `resizable: false` 설정
...
3. OCR 모드로 전환 시 → 자동으로 음성 스트리밍 중지 후 OCR 모드 활성화

### 트레이 메뉴 동작

1. "영역 지정" 클릭 → OCR 모드로 직접 전환 (`switchToOcrMode()` 호출)
2. "영역 재지정" 클릭 → OCR 모드로 직접 전환 (`switchToOcrMode()` 호출)
3. 음성 모드에서 트레이로 OCR 모드 전환 시 → 음성 스트리밍 자동 중지

### 성능 최적화

- OCR 캡처 간격: 800ms (성능 최적화)
- 썸네일 크기 최적화: ROI 영역 중심으로만 캡처하여 메모리 사용량 감소
- 서버 AI 분석 타임아웃: 3초 (재시도 1회)
- 비동기 처리로 지연 최소화
- 로그 최적화: 유해 표현 감지 위주로 출력
- 중복 텍스트 필터링: OCR 및 서버 분석 중복 방지
- 단일 인스턴스 보장: 중복 실행 방지로 리소스 절약

## 최근 개선 사항 (2025-11-26)

### Registration-Free COM 문제 해결 ✅

- ✅ 데스크탑과 노트북 환경 차이 분석 및 해결
- ✅ `OnVoiceComBridge.exe.manifest`: 데스크탑 환경과 동일하게 설정 (DLL 참조만 포함)
- ✅ `OnVoiceAudioBridge.dll.manifest`: 데스크탑 환경과 동일하게 설정 (COM 클래스 정보 포함)
- ✅ side-by-side 구성 오류 해결

### 모드 전환 개선 ✅

- ✅ 음성 모드에서 OCR 모드로 전환 시 자동으로 음성 스트리밍 중지
- ✅ `switchToOcrMode()` 함수 분리: OCR 모드 전환 로직을 재사용 가능한 함수로 분리
- ✅ 트레이 메뉴에서 OCR 모드로 직접 전환 가능
- ✅ 메인 윈도우 자동 숨김 로직 개선
